...
async def test_pwm_freq(dut):
    ...
    # Enable output 0 and PWM on output 0
...
async def test_pwm_duty(dut):
    ...
    await with_timeout(FallingEdge(dut.uo_out), 1_000_000, "ns")
    t2 = cocotb.utils.get_sim_time(units="ns")
    ...
    dut._log.info(f"Measured duty = {duty_percent}%")
    dut._log.info(f"Expected duty = {expected_duty}%")
